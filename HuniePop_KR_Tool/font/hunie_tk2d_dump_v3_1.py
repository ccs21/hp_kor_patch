...
def pptr_tuple(pptr: Any) -> Optional[Tuple[int, int]]:
    # Unity 4.x typetree PPtr usually uses m_FileID/m_PathID
...
def main():
    ...
    for path_id, obj in env.objects.items():
        ...
        # UnityPy typetree for arrays can differ; keep safe
        ...
        rec = {
            "mono_pathid": path_id,
            "mono_name": tree.get("m_Name", ""),  # often empty
            "version": version,
        ...
        if args.verbose:
            print(f"[HIT] mono={path_id} lineH={line_height} chars={chars_count} "
