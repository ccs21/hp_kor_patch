...
namespace FontHook
{
    ...
    internal class FontHookRunner : MonoBehaviour
    {
        ...
        private static readonly int[] BOLD_SIZES = new int[] { 16, 18, 20, 22, 30 };
        ...
        private void LoadAllPacks()
        {
            ...
            // bold sizes (closest-match fallback relies on these when available)
            TryLoadPack("b16");
            TryLoadPack("b18");
            TryLoadPack("b20");
            TryLoadPack("b22");
        ...
        private void TryLoadPack(string key)
        {
            try
            {
                ...
                // Hiero(BMFont) may output multi-page atlases (page 0/1/2...) declared inside the .fnt.
                // We only require the .fnt to exist; BMFontPack.LoadFromFnt will load and (if needed) merge pages.
                if (!File.Exists(fnt))
                {
...
bottom += 14f;
    ...
    internal class BMFontPack
    {
        ...
        // Load BMFont from .fnt, supporting multi-page atlases (Hiero pages=2 etc.)
        // If multiple pages exist, they are merged horizontally into a single Texture2D,
        // and glyph x is offset by page width.
        public static BMFontPack LoadFromFnt(string key, string fntPath, string baseDir)
        {
            ...
            using (StringReader sr = new StringReader(txt))
            {
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    if (line.StartsWith("common "))
                    {
                        ...
                        declaredPages = GetInt(kv, "pages", 1);
                    }
            ...
            // Determine page textures to load.
            // If the .fnt didn't declare pages, fallback to key.png (legacy single-page layout).
            List<int> pageIds = new List<int>();
            if (pageFiles.Count > 0)
            {
                foreach (int pid in pageFiles.Keys) pageIds.Add(pid);
                pageIds.Sort();
            }

            if (pageIds.Count == 0)
            {
                string fallback = P.Join2(baseDir, key + ".png");
                if (!File.Exists(fallback))
                {
                    Log.I("[BMFont] missing png (fallback): " + fallback);
                    return null;
                }

                pack.Texture = LoadPngTexture(fallback);
            ...
            // Load page textures
            Texture2D[] pages = new Texture2D[pageIds.Count];
            int[] pageIdByIndex = new int[pageIds.Count];
            ...
            for (int i = 0; i < pageIds.Count; i++)
            {
                ...
                string file = pageFiles[pid];
                string p = P.Join2(baseDir, file);

                if (!File.Exists(p))
                {
                    Log.I("[BMFont] missing page png: " + p);
                    return null;
                }

                Texture2D t = LoadPngTexture(p);
            ...
            // Merge pages horizontally into one texture
            int totalW = 0;
            int maxH = 0;
            for (int i = 0; i < pages.Length; i++)
            {
                totalW += pages[i].width;
                if (pages[i].height > maxH) maxH = pages[i].height;
            }

            Texture2D merged = new Texture2D(totalW, maxH, TextureFormat.ARGB32, false);

            // Copy pixels (Unity 4.2 safe via reflection)
            int xoff = 0;
            Dictionary<int, int> pageXOffset = new Dictionary<int, int>();
            for (int i = 0; i < pages.Length; i++)
            {
                Texture2D src = pages[i];
                int pid = pageIdByIndex[i];

                pageXOffset[pid] = xoff;

                if (!BlitPixels(merged, xoff, 0, src))
                {
                    Log.E("[BMFont] page blit failed pid=" + pid);
                    return null;
                }
            ...
            merged.name = key;
            merged.filterMode = FilterMode.Point;
            merged.wrapMode = TextureWrapMode.Clamp;
            merged.anisoLevel = 0;

            pack.Texture = merged;

            // Update scaleW/H to the merged atlas size (critical for texelSize)
            pack.ScaleW = merged.width;
            pack.ScaleH = merged.height;

            // Offset glyph x by page placement
            foreach (KeyValuePair<int, BMFontGlyph> kv in pack.Glyphs)
            {
                BMFontGlyph g = kv.Value;
                int off;
                if (pageXOffset.TryGetValue(g.page, out off))
                {
                    g.x += off;
                    g.page = 0;
                }
            ...
            Log.I("[BMFont] merged pages=" + pages.Length + " => " + merged.width + "x" + merged.height + " key=" + key);
        ...
        private static bool BlitPixels(Texture2D dst, int x, int y, Texture2D src)
        {
            try
            {
                // Prefer Color32 path
                MethodInfo miGet = typeof(Texture2D).GetMethod("GetPixels32", new Type[0]);
                MethodInfo miSet = typeof(Texture2D).GetMethod("SetPixels32", new Type[] { typeof(int), typeof(int), typeof(int), typeof(int), typeof(Color32[]) });

                if (!object.ReferenceEquals(miGet, null) && !object.ReferenceEquals(miSet, null))
                {
                    object pix = miGet.Invoke(src, null); // Color32[]
                    miSet.Invoke(dst, new object[] { x, y, src.width, src.height, pix });
                    return true;
                }

                // Fallback: Color[]
                MethodInfo miGetC = typeof(Texture2D).GetMethod("GetPixels", new Type[0]);
                MethodInfo miSetC = typeof(Texture2D).GetMethod("SetPixels", new Type[] { typeof(int), typeof(int), typeof(int), typeof(int), typeof(Color[]) });

                if (!object.ReferenceEquals(miGetC, null) && !object.ReferenceEquals(miSetC, null))
                {
                    object pix = miGetC.Invoke(src, null); // Color[]
                    miSetC.Invoke(dst, new object[] { x, y, src.width, src.height, pix });
                    return true;
                }
            }
            catch (Exception e)
            {
                Log.E("[BMFont] BlitPixels EX: " + e);
            }

            return false;
        }

        private static void ApplyTexture(Texture2D tex)
        {
            try
            {
                MethodInfo mi = typeof(Texture2D).GetMethod("Apply", new Type[] { typeof(bool), typeof(bool) });
                if (!object.ReferenceEquals(mi, null))
                {
                    mi.Invoke(tex, new object[] { false, false });
                    return;
                }
            }
            catch { }
...
internal class BMFontGlyph
{
    ...
    // BMFont 'page' index (0..n-1). Will be normalized to 0 after merging.
    public int page;
}
}
