...
def extract_strings_full(
    ...
    emit(("log", f"[STAT] objects_checked={checked} typetree_ok={typetree_ok} typetree_fail={typetree_fail} textasset_ok={textasset_ok} rows={len(rows)}"))
    return out_full
...
class App(tk.Tk):
    ...
    def _pipeline(self, mode: str, emit):
        ...
        asset_files.extend(sorted(data_dir.glob("*.assets")))
