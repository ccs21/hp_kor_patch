...
def _extract_sentence_candidates_from_bytes(blob: bytes, min_len: int, base_offset: int = 0) -> List[str]:
    """오브젝트 바이트 범위에서 사람이 읽을만한 문장 후보를 뽑습니다.
    - utf-8: 전체 디코드 후 패턴 매칭
    - utf-16le: '정렬(2바이트) + NUL 패턴' 기반으로 문자열을 찾아 디코드
      (Unity 4.2 MonoBehaviour 직렬화에서 문자열이 2바이트 정렬이 어긋난 위치에 있을 수 있어
       통째 decode(utf-16le)로는 앞글자가 깨지는 문제가 발생함)
    """
    out: List[str] = []
    ...
    # 2) utf-16le 기반 회수 (정렬/널 패턴 기반)
    #    - (printable_byte or 0xB7 '·') + 0x00 이 반복되는 구간을 문자열로 간주
    #    - 마지막은 \x00\x00(널 종료) 또는 비-패턴 바이트에서 끊김
    def _scan_utf16le_at_parity(parity: int):
        i = parity
        n = len(blob)
        while i + 1 < n:
            # 시작 조건: [printable|middle-dot] + 0x00
            b0 = blob[i]
            b1 = blob[i + 1]
            is_printable = (32 <= b0 <= 126) or (b0 == 0xB7)
            if b1 == 0x00 and is_printable:
                j = i
                # 연속 구간 확장
                while j + 1 < n:
                    c0 = blob[j]
                    c1 = blob[j + 1]
                    ok = (c1 == 0x00) and ((32 <= c0 <= 126) or (c0 == 0xB7))
                    if not ok:
                        break
                    j += 2
                # i..j 구간 디코드
                try:
                    s = blob[i:j].decode("utf-16le", errors="ignore")
                    if s:
                        _add_from_decoded(s)
                except Exception:
                    pass
    ...
    try:
        # Unity 4.2 직렬화 문자열은 파일 기준 2바이트 정렬(짝수 오프셋)에 저장되는 경우가 대부분입니다.
        # blob 시작(base_offset)과 합친 (base_offset + i)가 짝수인 위치만 스캔하여, 1바이트 어긋남으로 인한 앞글자 깨짐/누락을 방지합니다.
        parity = (base_offset & 1)
        _scan_utf16le_at_parity(parity)
    except Exception:
        pass
                            ...
                            if rng is not None:
                                ...
                                if asset_bytes is not None:
                                    ...
                                    if 0 <= s < e <= len(asset_bytes):
                                        ...
                                        cands = _extract_sentence_candidates_from_bytes(blob, min_len=cfg.min_len, base_offset=s)
                                        if cands:
                                            raw_fallback_ok += 1
                                            for t in cands:
