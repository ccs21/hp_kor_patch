...
def load_typetree_generator(game_root: Path, unity_ver: str):
    """TypeTreeGeneratorAPI 기반으로 로컬 게임에서 typetree 생성기를 로드합니다.
    HuniePop 2 resources.assets는 enable_type_tree=False(스트립)인 경우가 많아서,
    MonoBehaviour.parse_as_dict()가 동작하려면 typetree_generator를 붙여줘야 합니다.
    """
    gen = TypeTreeGenerator(unity_ver)
    # 로컬 게임(루트)에서 Managed/메타를 읽어 typetree를 생성
    gen.load_local_game(str(game_root))
    return gen

def load_resources_env(data_dir: Path, gen=None, use_typetree: bool = True):
    """resources.assets 로드 후(가능하면) typetree_generator를 주입합니다."""
    try:
    ...
    if gen is not None:
        # UnityPy는 env.typetree_generator에 generator가 붙어있으면 parse_as_dict가 훨씬 안정적으로 동작합니다.
        try:
...
def main():
    ...
    ap.add_argument("--game", default="", help="게임 루트(…\HuniePop 2 - Double Date). 미지정 시 --data의 상위 폴더를 사용")
    ...
    # game_root 결정: --game 미지정이면 --data의 상위 폴더를 게임 루트로 간주
    game_root = Path(args.game) if (args.game or "").strip() else data_dir.parent
    ...
    # unity version 확인 (resources.assets 헤더에서 읽기)
    env0 = UnityPy.load(str(data_dir / "resources.assets"))
    unity_ver = getattr(env0.file, "unity_version", "2019.4.3f1")
    enable_tt = getattr(env0.file, "_enable_type_tree", None)
    print(f"[INFO] unity_version={unity_ver} enable_type_tree={enable_tt}")

    # typetree generator 로드 후, env에 붙여서 parse_as_dict 안정화
    gen = load_typetree_generator(game_root, unity_ver)
    env = load_resources_env(data_dir, gen=gen, use_typetree=True)
